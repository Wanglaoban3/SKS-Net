...
class Resize(object):
    ...
    def padding_resize(self, image, target_size):
        target_size = target_size[0]
        channel = 1 if image.shape[-1] != 3 else 3
        # 获取图像原始尺寸
        ...
        if channel == 3:
            ...
            padded_image = np.zeros((target_size, target_size, channel), dtype=np.uint8)
        elif channel == 1:
    ...
    def __call__(self, image, label):
        if self.padding:
            image = self.padding_resize(image, self.size[::-1])
            label = self.padding_resize(label, self.size[::-1])
        else:
